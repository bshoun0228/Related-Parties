...
rp['OCC_BASE'] = rp['OCC'].astype(str)
rp['OCC_BASE'] = rp['OCC_BASE'].str.upper()
rp['OCC_BASE'] = rp['OCC_BASE'].str.strip()
rp['OCC_BASE'] = rp['OCC_BASE'].str.replace(r'\,', '', regex=True)
rp['OCC_BASE'] = rp['OCC_BASE'].str.replace(r'\.', '', regex=True)
...
#%% Drop only ESTATE if it is not part of REAL ESTATE
reallist = ['ESTATE']
rp['OCC_BASE'] = rp['OCC_BASE'].apply(lambda x: ' '.join([word for word in x.split() if word not in reallist]) if (('ESTATE' in x) & ('REAL ESTATE' not in x)) else x)

#%% do it again after cleaning to see what we've missed
common_words_2 = pd.DataFrame(Counter(" ".join(rp['OCC_BASE']).split()).most_common(200))
common_words_2.columns=['Word','Count']

#%%
rp_bus = rp[['OCC', 'OCC_BASE']]
rp_bus = rp_bus.rename(columns={'OCC': 'RP_OCC'})
...
matches['RATIO_BASE'] = matches.apply(lambda x: fuzz.ratio(x['LOAN_BASE'], x['OCC_BASE']), axis=1)

#%%
matches = matches[matches['RATIO_BASE']>=60]
matches['RATIO_ORDER'] = matches.apply(lambda x: fuzz.token_sort_ratio(x['LOAN_BASE'], x['OCC_BASE']), axis=1)
matches['RATIO_FULL'] = matches.apply(lambda x: fuzz.ratio(x['LOAN_BASE'], x['RP_OCC']), axis=1)

matches = matches[['RP_OCC','LOAN_NAME', 'LOAN_BASE', 'OCC_BASE', 'RATIO_BASE', 'RATIO_ORDER', 'RATIO_FULL']]
...
writer = pd.ExcelWriter('RelatedPartiesMatchingExample.xlsx', engine='xlsxwriter')
